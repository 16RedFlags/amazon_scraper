 ...
 */
const extractImageUrls = ($: any): string[] => {
  const imageUrls = $(SELECTORS.IMAGES)
    .map((_: any, imageEl: any) => $(imageEl).attr("src"))
 ...
 */
const extractProductAttributes = ($: any): ProductAttribute[] => {
  const attributeRowEls = $(SELECTORS.PRODUCT_ATTRIBUTE_ROWS).get();

  const attributeRows = attributeRowEls.map((rowEl: any) => {
 ...
 */
export const extractProductDetails = ($: any): ProductDetails => {
